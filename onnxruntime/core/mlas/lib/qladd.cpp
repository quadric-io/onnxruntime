...
//     )
// {
//     std::vector<float> ScaleValueVecA = {ScaleA};  // Create single-element vector
//     auto pairA = dataToQfp(ScaleValueVecA, -1, 32, false); // Returns std::make_pair(qfp, fracBits)
//     int fracBitsA = pairA.second;
//     int mulScaleA = fracBitsA - 2;
//     int64_t* fpScaleA = new int64_t;
//     *fpScaleA = static_cast<int64_t>(ScaleA * (1LL << fracBitsA));

//     std::vector<float> ScaleValueVecB = {ScaleB};  // Create single-element vector
//     auto pairB = dataToQfp(ScaleValueVecB, -1, 32, false); // Returns std::make_pair(qfp, fracBits)
//     int fracBitsB = pairB.second;
//     int mulScaleB = fracBitsB - 2;
//     int64_t* fpScaleB = new int64_t;
//     *fpScaleB = static_cast<int64_t>(ScaleB * (1LL << fracBitsB));

//     std::vector<float> ScaleValueVecC = {ScaleC};  // Create single-element vector
//     auto pairC = dataToQfp(ScaleValueVecC, -1, 32, false); // Returns std::make_pair(qfp, fracBits)
//     int fracBitsC = pairC.second;
//     int mulScaleC = fracBitsC - 2;
//     int64_t* fpScaleC = new int64_t;
//     *fpScaleC = static_cast<int64_t>(ScaleC * (1LL << fracBitsC));

//     std::cout << "A" << fracBitsA << std::endl;
//     std::cout << "B" << fracBitsB << std::endl;
...
//     if (IsScalarB) {
//         ValueB = (int64_t(InputB[0]) - ZeroPointB) * (*fpScaleC);
//     }

//     // here
//     for (size_t n = 0; n < N; n++) {
//         int64_t ValueA = (*fpScaleA) * (int32_t(InputA[n]) - ZeroPointA);
//         if (!IsScalarB) {
//             ValueB = (*fpScaleB) * (int32_t(InputB[n]) - ZeroPointB);
//         }
//         int64_t ValueC = (ValueA + ValueB) / (*fpScaleC);
//         // ValueC = ValueC >> mulScaleC;
//         // ValueC = customRound<2>(static_cast<int32_t>(ValueC));
